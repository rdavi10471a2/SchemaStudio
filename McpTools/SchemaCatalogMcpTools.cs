...
public sealed class SchemaCatalogMcpTools
{
    ...
    public async Task<object> ListDatabasesAsync(
        [Description("When true, include suggested next tool calls in the response. Defaults true in debug builds and false otherwise.")] bool? includeNext = null)
    {
        try
        {
            var showNext = ResolveIncludeNext(includeNext);
            var databases = await repository.GetDatabasesAsync();
            return Ok(databases.Select(x => ProjectDatabase(x, showNext)).ToList());
        }
        catch (Exception ex)
        {
        ...
        [Description("When true, include suggested next tool calls in the response. Defaults true in debug builds and false otherwise.")] bool? includeNext = null)
    {
        try
        {
            ...
            var domains = await repository.GetDomainsAsync(databaseId);
            return Ok(new
            {
        ...
        [Description("When true, include suggested next tool calls in the response. Defaults true in debug builds and false otherwise.")] bool? includeNext = null)
    {
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(domain) &&
                !domains.Any(x => string.Equals(x.Domain, domain, StringComparison.OrdinalIgnoreCase)))
            {
                return Fail(
                    "domain_not_found",
                    ...
                    details: new
                    {
                        availableDomains = domains.Select(x => x.Domain).ToList()
                    });
            }

            var objects = await repository.GetSchemaObjectsAsync(databaseId, domain);
            return Ok(new
            {
                database = ProjectDatabase(database, showNext),
                domain = string.IsNullOrWhiteSpace(domain) ? null : domain.Trim(),
                objects = objects.Select(x => ProjectSchemaObject(x, showNext)).ToList()
            });
        }
        catch (Exception ex)
        {
    ...
    private static object Fail(string code, string message, Exception? exception = null, object? details = null) => new
    {
        ok = false,
        data = (object?)null,
        error = new
    ...
    {
        if (includeNext.HasValue)
        {
            return includeNext.Value;
        }
