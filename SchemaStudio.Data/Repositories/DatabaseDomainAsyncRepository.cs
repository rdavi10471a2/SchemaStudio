...
public sealed class DatabaseDomainAsyncRepository
{
    public const string UnknownDomainName = "Unknown";

    private readonly string _connectionString;

    public DatabaseDomainAsyncRepository(string connectionString)
    {
...
""";
    ...
    public async Task<IReadOnlyList<DatabaseDomainDefinition>> GetByIdsAsync(IEnumerable<int> databaseDomainIds)
    {
        await using (var connection = new SqlConnection(_connectionString))
        {
            const string sql = """
SELECT
    ...
    Description
FROM dbo.DatabaseDomain
WHERE DatabaseDomainId IN @databaseDomainIds
ORDER BY Domain;
""";

            var rows = await connection.QueryAsync<DatabaseDomainDefinition>(sql, new { databaseDomainIds });
            return rows.ToList();
        }
    }
