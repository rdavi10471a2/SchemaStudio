...
[FileVersion("1.0")]
[AIFileContext("SchemaStudio.Data/Repositories/DatabaseAsyncRepository.cs", "Async read/write repository for Schema Studio database metadata records.", Responsibilities = "Loads and maintains dbo.Databases rows for maintenance screens and downstream schema tools.", Nuances = "Applies small additive metadata table upgrades before database reads and writes so UI fields can roll out without a separate migration step.", LastReviewed = "2026-05-21")]
public sealed class DatabaseAsyncRepository
{
    private readonly string _connectionString;

    public DatabaseAsyncRepository(string connectionString)
    {
...
""";
    ...
    public async Task<IReadOnlyList<DatabaseDefinition>> GetByIdsAsync(IEnumerable<int> databaseIds)
    {
        await using (var connection = new SqlConnection(_connectionString))
        {
            await EnsureDatabaseMetadataColumnsAsync(connection);
...
""";
            ...
            var rows = await connection.QueryAsync<DatabaseDefinition>(sql, new { databaseIds });
            return rows.ToList();
        }
    }
...
public sealed class DatabaseLookupRelationshipAsyncRepository
{
    private readonly string _connectionString;
    private readonly string _metadataDatabaseName;

    public DatabaseLookupRelationshipAsyncRepository(string connectionString)
    {
        _connectionString = connectionString;
